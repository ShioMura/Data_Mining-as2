...
selected_features_path = OUTPUT_DIR / "selected_features_extra_v2_top70.csv"

submission_path = OUTPUT_DIR / "submission_model_b_lgbm_ranker_extra_v2_pruned_top70_seed_ensemble.csv"
val_score_path = OUTPUT_DIR / "model_b_val_scores_extra_v2_pruned_top70_seed_ensemble.csv"
test_score_path = OUTPUT_DIR / "model_b_test_scores_extra_v2_pruned_top70_seed_ensemble.csv"
importance_path = OUTPUT_DIR / "model_b_feature_importance_extra_v2_pruned_top70_seed_ensemble.csv"

print("ROOT_DIR:", ROOT_DIR)
print("DATA_DIR:", DATA_DIR)
print("Train feature path exists:", train_path.exists())
print("Test feature path exists:", test_path.exists())
print("Selected features path exists:", selected_features_path.exists())


# ============================================================
# 1. Load data and selected top70 features
# ============================================================
...
# Safety check
missing_train = [c for c in selected_features if c not in train_fe.columns]
missing_test = [c for c in selected_features if c not in test_fe.columns]

if missing_train:
    raise ValueError(f"Missing selected features in train: {missing_train}")

if missing_test:
    raise ValueError(f"Missing selected features in test: {missing_test}")
...
# ============================================================
# 3. Sort by query and create groups
# ============================================================
...
# ============================================================
# 5. Train seed models on train/validation
# ============================================================
...
for seed in seeds:
    print("=" * 70)
    print("Training pruned top70 validation model with seed =", seed)
    print("=" * 70)
...
print("=" * 70)
print("PRUNED TOP70 SEED ENSEMBLE VALIDATION RESULT")
print("Seeds:", seeds)
...
val_scores = val_part[
    ["srch_id", "prop_id", "relevance", "ensemble_score"]
].copy()
...
# ============================================================
# 8. Train final seed models on full train and predict test
# ============================================================
...
for seed, n_estimators in zip(seeds, best_iterations):
    print("=" * 70)
    print("Training final pruned top70 full model with seed =", seed)
...
# ============================================================
# 9. Create final ensemble submission
# ============================================================
