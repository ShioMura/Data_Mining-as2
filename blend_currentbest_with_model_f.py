...
result_path = OUTPUT_DIR / "blend_currentbest_model_f_results.csv"
...
# current best:
# 0.95 * current_best_base_norm + 0.05 * v2_single_norm
#
# where current_best_base:
# 0.80 * weighted_seed + 0.20 * top70_seed
# ============================================================
...
# For Model F, use click_ranker_score if available because it was best
if "click_ranker_score" in model_f_val.columns:
    model_f_val = model_f_val.rename(columns={"click_ranker_score": "model_f_score"})
else:
    f_score_col = find_score_col(model_f_val)
    model_f_val = model_f_val.rename(columns={f_score_col: "model_f_score"})
...
val = val.merge(
    model_f_val[["srch_id", "prop_id", "model_f_score"]],
    on=["srch_id", "prop_id"],
    how="inner"
)
...
val["model_f_score_norm"] = normalize_within_search(val, "model_f_score")
...
# ============================================================
# 2. Search small Model F weights
# ============================================================

results = []

for w_current in np.arange(0.90, 1.0001, 0.01):
    w_f = 1.0 - w_current
    ...
    val["blend_score"] = (
        ...
        + w_f * val["model_f_score"]
    )

    raw_ndcg = mean_ndcg_at_5(val)

    print("Raw NDCG@5:", raw_ndcg)
    ...
    val["blend_score"] = (
        ...
        + w_f * val["model_f_score_norm"]
    )

    norm_ndcg = mean_ndcg_at_5(val)

    print("Norm NDCG@5:", norm_ndcg)
