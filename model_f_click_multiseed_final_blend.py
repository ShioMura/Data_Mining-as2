...
f_seed42_test_path = OUTPUT_DIR / "model_f_click_ranker_final_test_scores.csv"

test_score_path = OUTPUT_DIR / "model_f_click_multiseed_final_test_scores.csv"
submission_path = OUTPUT_DIR / "submission_currentbest_model_f_click_multiseed_blend.csv"
...
print("F seed42 test exists:", f_seed42_test_path.exists())
...
# ============================================================
# 3. Train final extra F click seeds
# seed42 already exists from previous script
# ============================================================
...
extra_seed_score_dfs = []

for seed, n_estimators in extra_seed_iters.items():
    print("=" * 70)
    print("Training final F click ranker seed =", seed)
    ...
    temp = test_sorted[["srch_id", "prop_id"]].copy()
    temp[score_col] = ranker.predict(X_test)

    extra_seed_score_dfs.append(temp)


# ============================================================
# 4. Load seed42 F click test score
# ============================================================

f42_test = pd.read_csv(f_seed42_test_path)

if "model_f_click_score" in f42_test.columns:
    f42_test = f42_test.rename(columns={"model_f_click_score": "f_click_seed42"})
else:
    score_col = find_score_col(f42_test)
    f42_test = f42_test.rename(columns={score_col: "f_click_seed42"})

f_scores = f42_test[["srch_id", "prop_id", "f_click_seed42"]].copy()

for temp in extra_seed_score_dfs:
    f_scores = f_scores.merge(
        temp,
        on=["srch_id", "prop_id"],
        how="inner",
    )

f_scores["f_click_weighted_b"] = (
    0.60 * f_scores["f_click_seed42"]
    + 0.20 * f_scores["f_click_seed2024"]
    + 0.20 * f_scores["f_click_seed3407"]
)

f_scores.to_csv(test_score_path, index=False)
print("Saved F multiseed test scores to:", test_score_path)


# ============================================================
# 5. Reconstruct current_pre_f test score
...
test = test.merge(
    f_scores[["srch_id", "prop_id", "f_click_weighted_b"]],
    on=["srch_id", "prop_id"],
    how="inner",
)
...
# 6. Final blend
# Validation best:
# 0.988 current_pre_f + 0.012 f_click_weighted_b
# ============================================================

test["blend_score"] = (
    0.988 * test["current_pre_f_score"]
    + 0.012 * test["f_click_weighted_b"]
)
